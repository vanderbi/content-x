...
### File and variable naming {#file-and-variable-naming}
...
### Data table structure {#data-table-structure}

Two distinct data table structures/formats may be used in different situations. 

The first is frequently called 'wide, tidy, or matrix' style and each column represents a single measured variable with each record (or row) representing one observation time and location. This format is generally prefered by statistical applications and is best described with EML metadata. 

The 'long' table format is characterized by having very few columns, typically a column for a location, a sampling time, one that holds all variable names and one that has the corresponding value, i.e., a key-value format. This format is preferred for maximum flexibility in adding variables at any time and avoiding many empty cells. E.g., a measurement being added five years into a study would create five years of empty cells in an existing dataset. 
...
New measurement variables can be added as columns or within a single column as attribute value pairs, where one column represents the measurement variable and its attributes are listed in adjacent columns (e.g. unit). Either approach requires the inclusion of new metadata.


### One value per cell {#one-value-per-cell}
...
Another recommendation is to avoid overloading a cell with composite information. Below is an example illustrating the issue of more than one piece of information per cell. The first table's **Location_ID** column is a composite of multiple variables. Selecting values from a variable requires parsing. The second table follows the best practice of one piece of information per cell, where the data are easily accessed.
...
Using a widely accepted and unambiguous datetime format maximizes the readability across software applications and integration with data from other repositories. The EDI Data Repository recommends the [ISO 8601 Standard](https://en.wikipedia.org/wiki/ISO_8601) whenever possible (e.g. YYYY-MM-DD hh:mm:ss). The full list of ISO 8601 permutations recognized by the EDI Data Quality Checker is available [here](https://github.com/EDIorg/ECC/blob/master/practices/dateTimeFormatString/dateTimeFormatString_list.csv). Remember to also specify the time zone and daylight savings observation practices during measurements.
...
## Quality Control of measured values {#quality-control-of-measured-values}
...
* Slope changes and steps - In time series, these may represent instrument drift.
...
New versions of the data package publication are clearly marked with an incrementing version number and release notes should be added to the metadata to communicate what has changed (e.g. implementation of new tests, or changes to existing tests).
